...
export function ContactProjectForm({ locale, initialType = '' }: ContactProjectFormProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (queryInterest) {
        if (queryInterest === 'website' || queryInterest === 'web_application' || queryInterest === 'web') {
          setFormData((prev) => ({ ...prev, interest: 'web' }));
        } else if (['mobile', 'unsure'].includes(queryInterest)) {
