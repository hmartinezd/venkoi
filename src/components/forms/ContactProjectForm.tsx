...
export function ContactProjectForm({
  ...
  const focusFirstError = (errObj: Record<string, string>) => {
    ...
    if (firstFieldKey && formRef.current) {
      ...
      if (optionalFieldNames.has(firstFieldKey) && optionalDetailsRef.current) {
        optionalDetailsRef.current.open = true;
      }
      const inputElement = formRef.current.querySelector<HTMLElement>(
        `[name="${firstFieldKey}"]`
      );
      if (inputElement) {
        inputElement.focus();
  ...
  return (
    ...
    >
      ...
      >
        <summary className="cursor-pointer rounded-xl px-4 py-4 outline-none transition hover:bg-surface-muted focus-visible:ring-2 focus-visible:ring-orange focus-visible:ring-offset-2 [&::-webkit-details-marker]:hidden">
          <span className="flex items-start justify-between gap-4">
            <span>
              <span className="block text-sm font-semibold text-ink">
                {t("optionalDetailsSummary")}
              </span>
              <span className="mt-1 block text-xs leading-relaxed text-foreground-muted">
                {t("optionalDetailsCue")}
              </span>
            </span>
            <span
              aria-hidden="true"
              className="text-orange-text transition group-open:rotate-45"
            >
              +
            </span>
          </span>
            ...
            <FormField label={t("phone")} error={errors.phone}>
              {(fieldProps) => (
                <input
                  ...
                  onChange={handleChange}
                  className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm text-ink transition focus:border-orange focus:outline-hidden"
                />
              )}
            </FormField>
            ...
            <FormField label={t("projectStage")} error={errors.project_stage}>
              {(fieldProps) => (
                ...
                >
                  ...
                  <option value="existing_product">
                    {t("stageOptions.existingProduct")}
                  </option>
                  <option value="needs_improvement">
                    {t("stageOptions.needsImprovement")}
                  </option>
                </select>
              )}
            </FormField>
