...
Resend production transactional delivery is configured and manually verified. Live English Contact and Request Access submissions confirmed both internal lead notifications and customer acknowledgements through the verified Venkoi sending domain. The application uses server-only `RESEND_API_KEY` and `RESEND_EMAIL_DOMAIN`, derives `Venkoi <notifications@{RESEND_EMAIL_DOMAIN}>`, sends internal notifications to the owner-configured `LEADS_NOTIFICATION_EMAIL`, and renders source-owned React Email templates—no Resend-hosted template IDs are used. Automated tests cover Standard Demo and Request Access in English and Spanish; Spanish production delivery has not been manually verified.
