...
const feature = read('src/components/home/ServeFeature.tsx');
...
assert.ok(
  header.indexOf('href={internalRoutes.productsServe}') < header.indexOf('href={internalRoutes.services}'),
...
assert.ok(home.indexOf('<ServeFeature') < home.indexOf('<ServicesSection'), 'The featured product should precede Services');
...
assert.ok(hero.includes('<ServeProductVisual type="hero"'), 'Homepage Hero should show the overview product preview');
...
assert.ok(feature.includes('<ServeProductVisual type="inventory"'), 'Featured product should show concrete inventory proof');
...
assert.match(feature, /eventName="serve_early_access_cta"/, 'Homepage access should preserve the access CTA analytics event');
assert.ok(
...
assert.ok(home.includes('buildServeVisualLabels(tVisuals)'), 'Homepage should use the shared visual-label builder');
...
for (const messages of [en, es]) {
  ...
  for (const key of ['theme1Title', 'theme2Title', 'theme3Title', 'theme4Title', 'theme5Title']) {
    assert.ok(messages.home.serve[key], `Homepage product outcome ${key} should exist`);
  }
  for (const state of ['available', 'early-access', 'launch-release', 'not-marketed']) {
    assert.equal(typeof messages.home.serve.availability[state], 'string');
  }
  assert.ok(messages.contactPage.productDemo.eyebrow, 'Contact should distinguish product intent');
}
assert.match(en.home.serve.theme1Desc, /review/i, 'Homepage should explain invoice review before posting');
assert.match(en.home.serve.theme2Desc, /receiving|waste|production/i, 'Homepage should explain why inventory changed');
assert.match(en.home.serve.theme3Desc, /Partial|Missing/i, 'Homepage should expose incomplete cost coverage');
assert.match(en.home.serve.theme4Desc, /par[\s\S]*packages[\s\S]*supplier/i, 'Homepage should explain reorder preparation logic');
assert.match(en.home.serve.theme5Desc, /price changes[\s\S]*variance[\s\S]*cost gaps/i, 'Homepage Owner View should identify signals requiring attention');
...
for (const file of [
  ...
  'src/components/home/ServeFeature.tsx',
